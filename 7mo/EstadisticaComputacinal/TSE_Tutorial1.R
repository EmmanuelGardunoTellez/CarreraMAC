...
# Hasta Agosto 2021 existen 17999 paqueterias que incluyen funciones y herramientas diversas
# para resolver problemas en distintas areas.

# https://cran.r-project.org/web/packages/available_packages_by_name.html

# Iniciando una sesion ordinaria en R
setwd("D:/Academia/Docencia/Cursos UNAM/Licenciatura/Temas selectos de Estadistica MAC")   	# fijar el directorio de trabajo

# Otra manera de escribir la ruta
#setwd(file.path("D:", "Academia","Docencia", "Cursos UNAM", "Licenciatura", "Temas Selectos de Estadistica MAC"))
...
# Si queremos "hacer nuestras propias notas" podemos elegir entre R notebook y R markdown
# Diferencia: es mas interactivo el notebook.
# En markdown todo el codigo es enviado a la consola de una sola vez mientras que
# en notebook se envia linea por linea, esto permite que la ejecucion se detenga
# si una linea conduce a un error.
...
rm()					# elimina objeto
help()        # ayuda sobre funciones y comandos
...
# f) Funciones especiales: disponibles en el paquete "gsl". 
#                                    Ejemplos: Airy, Bessel, Gamma, Laguerre, Legendre,
#                                    Psi, zeta etc.
# g) Otras funciones especiales disponibles en los paquetes: "orthopolynom", "pracma",
#                                                            "hypergeo", "specfun"

#++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
# 1. Creacion de objetos

b<- 32.98   # objeto b que consiste del valor 32.98
b<<- 32.98
...
?Syntax  # para checar la sintaxis
...
# 1.3 Factores (datos categoricos)

turno<- c("vespertino","nocturno","matutino")
turno<- factor(turno)
levels(turno)  # R acomoda por orden alfabetico

# EXTRA: reordenar factores de acuerdo a un criterio 

data(iris)
levels(iris$Species)
especie<- iris$Species
especie<- reorder(especie,iris$Sepal.Width,FUN=sum)
levels(especie)


#-----------------------------------------------------------------
# Ejercicio: ¿Cuál es la suma de los primeros 100 números enteros 
# positivos? La fórmula para la suma de enteros 1 a n es 
# n(n + 1)/2. 
...
# Raiz real de un numero negativo
(-8)^(1/3) # error!
# pero....
-8^(1/3)  # si da la respuesta correcta!
-(8)^(1/3)

# Asi que podemos hacerlo de manera mas general a traves de una funcion:

raiz.real <- function(x,pot) if(x < 0){-(-x)^(pot)}else{x^pot}

raiz.real(-8,1/3)
raiz.real(16,1/2)

# Pero si lo que necesitamos es la raiz de un numero complejo:

raiz.compleja<- function(x,pot) as.complex(x)^pot
...
round(1234.567,digits=3)
signif(1234.567,digits=3)   # redondea al numero especificado de digitos significativos
...
sec.=1; min_eq=60*sec.; hr.=60*min_eq
...
X<- Z*cos(Z)
Y<- Z*sin(Z)
scatterplot3d(X, Y, Z, col.axis = "blue",col.grid = "lightblue", main = "Hélice",type="l",color="red")
...
prod.esc=function(u,v) u%*%v  

u1=c(1,2,3)
u2=c(4,5,6)

prod.esc(u1,u2)  # esta bien?

# Mejor "forzamos" el resultado: as.numeric()

prod.esc=function(u,v) as.numeric(u%*%v)

prod.esc(u1,u2) # Mejor!

# Norma de un vector

vecnorm=function(v) sqrt(prod.esc(v,v))

vecnorm(u1); vecnorm(u2)
...
cross(c(8,9,2,1),c(1,0,-2,4))  # Oops!
...
matriz<- data.matrix(df)  # convierte a matriz numerica

# Otro ejemplo

df2 <- data.frame(Dia = seq(1:20),Estimado = rep(c(T,F),10),
                  Lectura = rnorm(20,5))

c(length(df2), ncol(df2), nrow(df2))
