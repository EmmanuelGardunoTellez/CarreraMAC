...
float Func_Suma(float * numero,int veces){ // Suma de un numero por referencia
	float i, pasar=*numero;
	for(i=0;i<veces;i++){
		pasar=pasar+*numero;
	}
	return pasar;
...
float Func_Resta(float * numero,int veces){ //Resta de un numero por referencia
	float i, pasar=*numero;
	for(i=0;i<veces;i++){
		pasar=pasar-*numero;
	}
...
float Func_Div(float * numero, int veces){ //Division de un numero por referencia
	float div=*numero, i;
	for(i=0;i<veces;i++){
		div= div/(*numero);
	}
...
float dfx (x px, int val){ //Evaluador de derivada de la funcion
	float p;
	p= px.x5*5*(pow(val,4))+px.x4*4*(pow(val,3))+px.x3*3*(pow(val,2))+px.x2*2*(pow(val,1))+px.x1*1*(pow(val,0));
	return p;
}
/*Funcion principal*/
void main(){
	...
	float num; 
	FILE *archivo = fopen("SumaRestaDivision.txt", "w"); //Abre documento txt
	printf("El valor inicial es el nuero que das de valor.\n");
	printf("De un valor: ");
	scanf("%f", &num);
	printf("Cuantas veces lo queres sumar, restar y dividir entre si mismo: ");
	scanf("%i",&ves);
	/*Los fprint almacenan los datos en el texto*/
	printf("La suma del numero es: %f\n",Func_Suma(&num,ves));
	fprintf(archivo, "La suma del numero es: %f\n",Func_Suma(&num,ves));
	printf("La suma del numero es: %f\n",Func_Resta(&num,ves));
	fprintf(archivo, "La resta del numero es: %f\n",Func_Resta(&num,ves));
	printf("La division del numero es: %f\n",Func_Div(&num,ves));
	fprintf(archivo, "La suma del numero es: %f\n",Func_Div(&num,ves));
	fclose(archivo);
	
	x px; //Estructura guardada con typedef, y mostrada de manera sencilla
	FILE *archivo2 = fopen("F(x).txt", "w"); //Abreo otro archivo
	/*Guarda polinomio*/
	...
	printf("Dime de que valor a que valor se evaluara px y la derivada de px:\n");
	...
	fprintf(archivo2, "num\t\tf(x)\t\tf(x)'\n");
	for(i=ini;i<ter+1;i++){
		fprintf(archivo2, "%i\t\t%f\t\t%f\n",i,fx(px,i),dfx(px,i));
	}
