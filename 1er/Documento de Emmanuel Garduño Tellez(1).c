...
void quitar_productos(char productos[C][P],int cantidad[C],char codigos[C][CO] ){
	...
	printf("Productos que puede quitar del carro de esta categoria:\n");
	for(i=0; i<20; i++)
	{
		if(cantidad[i]>0)  //Muestra cuantos productos compra por categoria
		{
	...
	if(res==1){
	do{ 
		printf("Escriba la ultima parte del código del producto a dejar:");
		scanf("%i",&deja);
...
//	portada();
	...
	printf("%.2f \n", total);
	printf("Selección de metodo de pago.\n");
	system("Pause");
}
