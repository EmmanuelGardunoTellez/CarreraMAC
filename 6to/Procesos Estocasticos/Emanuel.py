...
def exaustivas(elec, data, funimp, funpol, funest, funcos):
    ...
    ctt=data[4]# COSTOS
    ct=data[5]# DESICIONES APLICABLES A CADA ESTADO
    ...
    descu=float(input())
    poliOp=np.zeros((itera,len(ss)))
    V =np.zeros((itera,len(ss)))
    n=0
    ...
    while n < itera:
      if (n==0):
        ...
        for i in range(len(sk)):
          print('V_',i,'^',n+1)
          AuxM=np.zeros((len(sk), len(ct[i])))
          for k in ct[i]:
            aux=0
            for j in range(len(ss)):
              print(pks[k][i][j],'*',V[n-1][j])
              aux=aux + pks[k][i][j]*V[n-1][j]
            aux = descu*aux + ctt[i][k]
            V[n][k]=aux
            print(V[n])
            print(ctt[i][k])
            print('El minimo es:',min(V[n]))
            if (V[n][k]==min(V[n])):
              poliOp[n][i]=k+1
