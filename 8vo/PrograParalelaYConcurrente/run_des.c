...
int main(int argc, char* argv[]) {
	...
	if (argc < 2) { //seleccione que accion debo de hacer
	...
	if (strcmp(argv[1], ACTION_GENERATE_KEY) == 0) { // Generate key file
		...
		if (!key_file) { //no se abrre el archivo para crear llave
		...
		if (bytes_written != DES_KEY_SIZE) {//Error en el archivo de salida
		...
		if (!key_file) { //no se abrre el archivo para leer llave
		...
		if (bytes_read != DES_KEY_SIZE) { //llave de tamaño invalido
		...
		if (!input_file) { //Error al abrir el mensaje de entrada
		...
		if (!output_file) { //Erro al abrir el archico de salida
		...
		start = clock();
		generate_sub_keys(des_key, key_sets); //generamos las 16 subllaves
		finish = clock();
